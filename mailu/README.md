...
| `api.enabled`                                 | Enable the creation of the initial account                                                                                             | `false`         |
| `api.web`                                     | Enable access to the REST API                                                                                                          | `"/api"`        |
| `api.token`                                   | Token for REST API; ignored if using existing secret; if empty, a random token will be generated and saved in a secret                 | `""`            |
| `api.existingSecret`                          | Name of the existing secret to retrieve the REST API token                                                                             | `""`            |
| `api.existingSecretPasswordKey`               | Name of the key in the existing secret to use for the REST API token                                                                   | `""`            |
